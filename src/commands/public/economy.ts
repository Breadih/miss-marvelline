  ...
  async execute(interaction) {
    ...
    switch (subcommand) {
      case "create": {
        ...
        const embedIntroduction = new EmbedBuilder().setDescription(`Hey! Welcome to the economy system! In this system, you'll be held responsible to make your economy work and be the best. The rules are simple:\n* Current Rate: \n  * Each person that starts the economy, will have a rate of 0.5, being **1 the highest, therefore the best** currency, and **0, the worst economy**. Your server members must work in order to make the server economy the best.\n * Compare your Guild Coin to others: \n  * In the meanwhile you build your empire in your server, you might wanna take a look in comparing economies (so then you can flex to your friends :3) about how good your economy is, or not :p. `).setColor(colors.MainColour);
        ...
        await interaction.editReply({ embeds: [embedIntroduction], components: [ActionRow.toJSON()] });
